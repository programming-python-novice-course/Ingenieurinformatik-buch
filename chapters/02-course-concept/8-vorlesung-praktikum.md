...
| **P2**<br>if-else | **Kapitel 08**: Datentypen (Wiederholung)<br>**Kapitel 10**: Kontrollstrukturen<br>**Kapitel 09**: Eingaben/Ausgaben (Wiederholung) | • Fallunterscheidungen: if, elif, else (Kap. 10)<br>• Vergleichsoperatoren (Kap. 10)<br>• Formatierte Ausgabe mit f-Strings (Kap. 09) | **Wichtig**: Kapitel 10 behandelt sowohl Fallunterscheidungen als auch Schleifen zusammen. Lesen Sie das gesamte Kapitel, auch wenn P2 nur Fallunterscheidungen behandelt. |
| **P3**<br>Schleifen | **Kapitel 08**: Datentypen (Listen vertiefen)<br>**Kapitel 10**: Kontrollstrukturen (Schleifen)<br>**Kapitel 09**: Eingaben/Ausgaben (Wiederholung) | • Schleifen: while, for (Kap. 10)<br>• Listen (list) - vollständig (Kap. 08)<br>• Durchlaufen von Listen mit for (Kap. 10 + 08)<br>• Tabellarische Ausgabe | Vertiefen Sie jetzt die Listen aus Kapitel 08. Kombinieren Sie Schleifen (Kap. 10) mit Listen (Kap. 08). |
| **P4**<br>Funktionen (Motivation) | **Kapitel 11**: Funktionen | • Motivation für Funktionen<br>• Code-Strukturierung<br>• Parameter und Rückgabewerte<br>• Lokale Variablen | Lesen Sie Kapitel 11 vollständig. Die Motivation und das Verständnis für Funktionen stehen hier im Vordergrund. |
| **P4a**<br>Funktionen definieren | **Kapitel 11**: Funktionen (vertiefen) | • Funktionsdefinition mit def<br>• Parameter vs. lokale Variablen<br>• Rückgabewerte (return)<br>• Übergang von Skript zu Funktionsaufrufen | Vertiefen Sie die praktische Anwendung aus Kapitel 11. Üben Sie das Definieren eigener Funktionen. |
| **P5**<br>Listen | **Kapitel 08**: Datentypen (Listen)<br>**Kapitel 10**: Kontrollstrukturen<br>**Kapitel 11**: Funktionen (Wiederholung) | • Listen vollständig (Kap. 08)<br>• Verschachtelte Schleifen (Kap. 10)<br>• Systematisches Kombinieren von Listenelementen<br>• Funktionen mit Listen (Kap. 11 + 08) | Kombinieren Sie alle bisherigen Konzepte: Listen, Schleifen und Funktionen. Wiederholen Sie die relevanten Abschnitte aus den vorherigen Kapiteln. |
...
Im Buch sind **Fallunterscheidungen (if/elif/else) und Schleifen (while/for) in einem Kapitel** (Kapitel 10) zusammengefasst, weil beide Konzepte die Programmausführung steuern. Das Praktikum teilt diese Inhalte auf (P2: nur Fallunterscheidungen, P3: nur Schleifen), aber es ist sinnvoll, **Kapitel 10 komplett zu lesen**, um beide Konzepte im Zusammenhang zu verstehen.
...
2. Lesen Sie **Kapitel 10** vollständig - auch wenn nur Fallunterscheidungen behandelt werden, verstehen Sie so den Zusammenhang mit Schleifen
...
2. Lesen Sie **Kapitel 10** - jetzt besonders die Schleifen (while, for)
...
**Vorbereitung:**
1. Lesen Sie **Kapitel 11** vollständig
...
2. Wiederholen Sie **Kapitel 10** (verschachtelte Schleifen)
3. Wiederholen Sie **Kapitel 11** (Funktionen mit Listen)
...
**F: Muss ich Kapitel 10 komplett lesen, auch wenn P2 nur if/else behandelt?**  
...
**F: Soll ich Kapitel 11 (Funktionen) vor oder nach Kapitel 10 (Kontrollstrukturen) lesen?**  
A: Im Buch kommt Kapitel 10 (Kontrollstrukturen) vor Kapitel 11 (Funktionen), was didaktisch sinnvoll ist. Für das Praktikum macht es Sinn, Kapitel 10 zuerst zu lesen (P2/P3) und dann Kapitel 11 (P4/P4a). Die Buchreihenfolge entspricht jetzt der Praktikumsreihenfolge!
