...
# Kompaktes JSON-Preview (erste n Werte pro Station), damit die Ausgabe nicht riesig wird
...
```{code-cell} python3
def histogram(station_dict, bins=8, width=30):
    """
...
for station in data.keys():  # oder kurz: for station in data:
    print(f"\nHistogramm: {station}")
