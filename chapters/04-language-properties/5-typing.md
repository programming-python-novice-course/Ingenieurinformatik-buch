...
x = 3 
x = 4.0 
Python erkennt automatisch dass es ist eine ganze zahl oder eine gleitkommanzahl und speichert das entsprechende objekt möglichst effizient (wenige nullen) für uns ab. Da der Typ während der Laufzeit bestimmt wird, spricht man von dynamischer Typisierung.

## Starke typisierung

Python ist außedem stark typisiert (stark und dynamische Typisierung), d.h. dass der Typ einer ?? nicht einfach umgewandelt werden kann.
In einer schwach typisierte Sprache wie perl oder javascript ist folgendens möglich:
x = "5"
